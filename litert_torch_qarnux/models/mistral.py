"""
Mistral Model Architecture Definition.

Implements a minimal MistralForCausalLM model in pure PyTorch.
Mistral models share the same architecture as Llama (RMSNorm, RoPE,
SwiGLU MLP) but with a key difference: they use Grouped Query Attention (GQA)
with sliding window attention and do not tie embedding and output weights.
"""
...
class MistralAttention(nn.Module):
    """GQA attention with sliding window for Mistral."""
    ...
    def __init__(
        ...
        self.input_layernorm = MistralRMSNorm(hidden_size, eps=rms_norm_eps)
        self.rotary_emb = MistralRotaryEmbedding(head_dim, max_position_embeddings)
...
class MistralMLP(nn.Module):
    """SwiGLU MLP for Mistral."""

    def __init__(self, hidden_size: int, intermediate_size: int, rms_norm_eps: float = 1e-6):
        super().__init__()
        self.gate_proj = nn.Linear(hidden_size, intermediate_size, bias=False)
        self.up_proj = nn.Linear(hidden_size, intermediate_size, bias=False)
        self.down_proj = nn.Linear(intermediate_size, hidden_size, bias=False)
        self.post_attention_layernorm = MistralRMSNorm(hidden_size, eps=rms_norm_eps)
...
class MistralForCausalLM(BaseModel):
    """Full Mistral Causal Language Model."""
    ...
    def build(self) -> nn.Module:
        ...
        for _ in range(num_layers):
            layer = nn.ModuleDict({
                "self_attn": MistralAttention(hidden_size, num_heads, num_kv_heads, head_dim, rms_norm_eps),
                "mlp": MistralMLP(hidden_size, intermediate_size, rms_norm_eps),
            })
            layers.append(layer)
        ...
        model = nn.ModuleDict({
            ...
            "norm": MistralRMSNorm(hidden_size, eps=rms_norm_eps),
            "lm_head": nn.Linear(hidden_size, self.metadata.vocab_size, bias=False),
        })

        logger.info(
            "Built Mistral model: layers=%d, hidden=%d, heads=%d, kv_heads=%d, vocab=%d",
    ...
    def load_weights(
        ...
        logger.info("Loaded %d weight tensors into Mistral model", len(tensor_map))
...
ModelRegistry.register("mistral", MistralForCausalLM)
